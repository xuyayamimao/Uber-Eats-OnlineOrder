-- 为了开发用，所以每次initialize的时候只要存在这些table就全部drop掉
...
(
-- serial是一种特殊的自动增长的int，每次insert的时候会auto-increment：1，2，3，4
-- primary key是数据中unique的可以用来identify这组数据的东西：必须是unique的
-- not null代表这一栏必须不可以是空的
    id         SERIAL PRIMARY KEY   NOT NULL,
--     email unique代表的是不可以有重复的email，如果这个email已经被用了就不可以再放进去
    email      TEXT UNIQUE          NOT NULL,
-- enabled是看顾客有没有被disabled掉
...
(
    id          SERIAL PRIMARY KEY NOT NULL,
--     customer-id unique指的是一个customer只能有一个cart
    customer_id INTEGER UNIQUE     NOT NULL,
--     numeric是一个数字，算钱的时候比较方便的一个format
    total_price NUMERIC            NOT NULL,
--     foreign key的两个作用：
--     1. 帮你删掉这个数据：这个table中的customer_id就是customers table中的id --> 可以发先如果一个customer是假的话，那么cart就会直接被删掉（on delete cascade)
--     2. insert：customer_id写了1000000，但是customers中的id最大就只到10，所以就是错的->throw foreign key violation
--     ->foreign key帮你enforce各个table之间的relations
