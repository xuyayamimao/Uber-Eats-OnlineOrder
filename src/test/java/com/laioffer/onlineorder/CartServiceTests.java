...
//@Mock 全部给test传的是假的repository，来自mockito library ->这样我们的testing就不会受database的影响
    @Mock
...
//@BeforeEach:在每个test case之前，全部创建一个新的cart service ->这些testing annotations全部都来自junit，junit不依赖于任何framework
    @BeforeEach
    void setup() {
        cartService = new CartService(cartRepository, menuItemRepository, orderItemRepository);
    }

//test for when order doesn't exist, should create order iem
    @Test
...
//test for when order already exist
    @Test
    void addMenuItemToCart_whenOrderAlreadyExist_shouldUpdateOrderItem() {
        ...
        Mockito.verify(orderItemRepository).save(newOrderItem);
        Mockito.verify(cartRepository).updateTotalPrice(cartId, 20.0);//expect total price = 20
    }
...
//remove all items, reset total price
    @Test
